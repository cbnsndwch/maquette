...
```json
{
  ...
  "privatePackages": {         // Don't version private packages
    "version": false,
    "tag": false
  }
}
...
**Note for Private Repositories**: npm provenance is disabled for private repositories since it only works with public repositories. See `PRIVATE-REPO-FIX.md` for details.
5. **Creates GitHub releases** with release notes
